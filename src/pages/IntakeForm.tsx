...
const conditions = [
  ...
  "Allergies / Sinusitis", "Sports Injuries", "Arthritis", "Fatigue / Low Energy",
...
const IntakeForm = () => {
  ...
  const onSubmit = async (data: IntakeFormData) => {
    ...
    setSubmitting(true);
    try {
      const { data: result, error } = await supabase.functions.invoke("submit-intake", {
        body: {
          ...
          primary_concern: data.primaryConcern, conditions: selectedConditions,
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
              <section className="bg-card rounded-2xl p-8 shadow-sm">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div><Label htmlFor="gender">Gender *</Label><select id="gender" {...register("gender")} className="mt-1 w-full h-10 rounded-lg border border-input bg-background px-3 text-base md:text-sm"><option value="">Select...</option><option value="male">Male</option><option value="female">Female</option><option value="non-binary">Non-binary</option><option value="prefer-not">Prefer not to say</option></select>{errors.gender && <p className="text-destructive text-sm mt-1">{errors.gender.message}</p>}</div>
              ...
              <section className="bg-card rounded-2xl p-8 shadow-sm">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div><Label htmlFor="emergencyContact">Name & Relationship *</Label><Input id="emergencyContact" {...register("emergencyContact")} placeholder="Jane Doe (Spouse)" className="mt-1" />{errors.emergencyContact && <p className="text-destructive text-sm mt-1">{errors.emergencyContact.message}</p>}</div>
                  <div><Label htmlFor="emergencyPhone">Phone *</Label><Input id="emergencyPhone" type="tel" {...register("emergencyPhone")} className="mt-1" />{errors.emergencyPhone && <p className="text-destructive text-sm mt-1">{errors.emergencyPhone.message}</p>}</div>
                </div>
              </section>
              ...
              <section className="bg-card rounded-2xl p-8 shadow-sm">
                ...
                <div className="space-y-6">
                  <div><Label htmlFor="primaryConcern">Primary Health Concern *</Label><Textarea id="primaryConcern" {...register("primaryConcern")} placeholder="Describe what brings you to seek acupuncture treatment..." className="mt-1 min-h-[100px]" />{errors.primaryConcern && <p className="text-destructive text-sm mt-1">{errors.primaryConcern.message}</p>}</div>
                  <div>
                    <Label className="mb-3 block">Conditions you're experiencing (select all that apply)</Label>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                      {conditions.map((c) => (
                        <label key={c} className={`flex items-center gap-2 p-3 rounded-lg border cursor-pointer transition-colors ${selectedConditions.includes(c) ? "border-primary bg-sage-light" : "border-border hover:border-primary/50"}`}>
                  ...
                  <div><Label htmlFor="medicalHistory">Medical History</Label><Textarea id="medicalHistory" {...register("medicalHistory")} placeholder="Past surgeries, hospitalizations, significant medical events..." className="mt-1" /></div>
                  <div><Label htmlFor="currentMedications">Current Medications & Supplements</Label><Textarea id="currentMedications" {...register("currentMedications")} placeholder="List any medications, vitamins, or supplements you take..." className="mt-1" /></div>
                  <div><Label htmlFor="allergies">Allergies</Label><Input id="allergies" {...register("allergies")} placeholder="Drug, food, or environmental allergies..." className="mt-1" /></div>
                  <div><Label htmlFor="previousAcupuncture">Have you received acupuncture before? *</Label><select id="previousAcupuncture" {...register("previousAcupuncture")} className="mt-1 w-full h-10 rounded-lg border border-input bg-background px-3 text-base md:text-sm"><option value="">Select...</option><option value="yes">Yes</option><option value="no">No</option></select>{errors.previousAcupuncture && <p className="text-destructive text-sm mt-1">{errors.previousAcupuncture.message}</p>}</div>
                  <div><Label htmlFor="referralSource">How did you hear about us?</Label><Input id="referralSource" {...register("referralSource")} placeholder="Google, friend, doctor referral..." className="mt-1" /></div>
                </div>
              </section>
